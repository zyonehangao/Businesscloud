 ...
 */
class TextTypeEditText @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
                                              defStyle: Int = android.R.attr.editTextStyle)
    : EditText(context, attrs, defStyle), View.OnFocusChangeListener, TextWatcher {
    //EditText右侧的删除按钮
    private var mClearDrawable: Drawable? = null
    private var mShowTextDrawable: Drawable? = null
    private var hasFocus: Boolean = false
    private var textShowTepe:Boolean =false
    init {
        init()
    }
    ...
    private fun init() {
        ...
        mShowTextDrawable = compoundDrawables[2]
        if (mClearDrawable == null) {
            mClearDrawable = resources.getDrawable(
                    R.drawable.error1)
        }

        if (mShowTextDrawable == null) {
            mShowTextDrawable = resources.getDrawable(
                    R.drawable.blink)
        }

        mClearDrawable!!.setBounds(0, 0, mClearDrawable!!.intrinsicWidth+30,
                mClearDrawable!!.intrinsicHeight)
        //获取图片显示的尺寸
        mShowTextDrawable!!.setBounds(10, 0, mShowTextDrawable!!.intrinsicWidth,
                mShowTextDrawable!!.intrinsicHeight)
        // 默认设置隐藏图标
    ...
    private fun setShowTextVisible(visible: Boolean) {
        val right =  mShowTextDrawable
        setCompoundDrawables(compoundDrawables[0],
                compoundDrawables[1], right, compoundDrawables[3])
    }
